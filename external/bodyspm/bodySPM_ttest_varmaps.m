function bspm = bodySPM_ttest_varmaps(cfg)

% Small modification to bodySPM_ttest_love:
% Here, also mean, standard deviation, and confidence interval
...
ci_l=zeros(length(in_mask),NC);
ci_h = zeros(length(in_mask),NC);
means= zeros(length(in_mask),NC);
stds = zeros(length(in_mask),NC);
dfdata=zeros(NC,1);
...
for condit=1:NC
    ...
    ci_l(:,condit) = CI(1,:); % lower
    ci_h(:,condit) = CI(2,:); % upper
    means(:,condit) = mean(tempdata); % means
    stds(:,condit) = std(tempdata);   % standard deviations
end
...
for condit=1:NC
    ...
    tvals(:,:,condit)=temp;
    % ci_l
    temp_ci_l=zeros(size(mask));
    temp_ci_l(in_mask)=ci_l(:,condit);
    temp_ci_l(find(~isfinite(temp_ci_l)))=0; % we set nans and infs to 0 for display
    ci_l_reshaped(:,:,condit)=temp_ci_l;
    % ci_h
    temp_ci_h=zeros(size(mask));
    temp_ci_h(in_mask)=ci_h(:,condit);
    temp_ci_h(find(~isfinite(temp_ci_h)))=0; % we set nans and infs to 0 for display
    ci_h_reshaped(:,:,condit)=temp_ci_h;
    % means
    temp_means=zeros(size(mask));
    temp_means(in_mask)=means(:,condit);
    temp_means(find(~isfinite(temp_means)))=0; % we set nans and infs to 0 for display
    means_reshaped(:,:,condit)=temp_means;
    % standard devations
    temp_stds=zeros(size(mask));
    temp_stds(in_mask)=stds(:,condit);
    temp_stds(find(~isfinite(temp_stds)))=0; % we set nans and infs to 0 for display
    stds_reshaped(:,:,condit)=temp_stds;
end
...
bspm.ttest.ci_l=ci_l_reshaped;
bspm.ttest.ci_h=ci_h_reshaped;
bspm.ttest.mean=means_reshaped;
bspm.ttest.std=stds_reshaped;
